  ...
  return (
    <div className="bg-gray-100 min-h-screen py-12">
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold text-center text-darkred mb-12">I Nostri Servizi</h1>
        
        <div className="space-y-12">
          {serviziOfferti.map((servizio, index) => (
            <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className="p-6">
                <h2 className="text-2xl font-semibold text-darkred mb-4">{servizio.titolo}</h2>
                <p className="text-gray-600 mb-6">{servizio.descrizione}</p>
                <h3 className="text-lg font-semibold text-darkred mb-3">Vantaggi:</h3>
                <ul className="space-y-2">
                  {servizio.vantaggi.map((vantaggio, idx) => (
                    <li key={idx} className="flex items-start">
                      <CheckCircleIcon className="w-5 h-5 text-lightred mr-2 flex-shrink-0 mt-1" />
                      <span>{vantaggio}</span>
                    </li>
                  ))}
                </ul>
              ...
              <div className="bg-gray-50 px-6 py-4">
                <Link to="/contatti" className="text-lightred font-semibold hover:text-darkred transition duration-300 flex items-center">
        ...
        <div className="mt-16 text-center">
          <h2 className="text-2xl font-bold text-darkred mb-4">Hai domande sui nostri servizi?</h2>
          <p className="text-gray-600 mb-8">Siamo qui per aiutarti a trovare la soluzione migliore per le tue esigenze fiscali.</p>
          <Link to="/contatti" className="bg-darkred text-white font-bold py-3 px-6 rounded-full hover:bg-lightred transition duration-300">
            Contattaci Ora
          </Link>
        </div>
      </div>
